...
Examples
--------
# Search for journal articles with >= 10 citations
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    search_parser.add_argument("--fields", default=_DEFAULT_FIELDS)
    _add_filter_args(search_parser)

    bulk_parser = subparsers.add_parser("search-bulk", help="Bulk search with token pagination")
    bulk_parser.add_argument("query", help="Keyword query")
    bulk_parser.add_argument("--max", type=int, default=100, metavar="N")
    bulk_parser.add_argument("--token", default=None)
    bulk_parser.add_argument("--sort", default=None)
    bulk_parser.add_argument("--fields", default=_DEFAULT_BULK_FIELDS)
    _add_filter_args(bulk_parser)

    paper_parser = subparsers.add_parser("paper", help="Fetch one paper by ID")
    paper_parser.add_argument(
        "id",
        help="S2 paper ID, DOI, CorpusId:..., ARXIV:..., PMID:..., MAG:..., ACL:..., etc.",
    )
    paper_parser.add_argument("--fields", default=_DEFAULT_FIELDS)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "paper":
            result = get_paper(paper_id=args.id, fields=args.fields)
            print(json.dumps(result, ensure_ascii=False, indent=2))
            return 0
